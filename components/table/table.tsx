        ...
        <table className="w-[801px] border-spacing-0 border-collapse md:w-[962px] ">
          <thead>
            <tr className="h-[40px] text-[12px] border-b text-left border-gray-20 bg-red-10 md:text-[14px] md:h-[50px] ">
              <th className="tb-head w-[186px] sticky left-0  md:w-[110px] bg-red-10 xl:relative">
                {type === 'applicantList' && '신청자'}
                {type === 'applyList' && '기업'}
              </th>
              <th className="tb-head w-[270px] md:w-[200px]">
                {type === 'applicantList' && '소개'}
                {type === 'applyList' && '일자'}
              </th>
              <th className="tb-head w-[180px]  md:w-[150px]">
                {type === 'applicantList' && '전화번호'}
                {type === 'applyList' && '시급'}
              </th>
              <th className="tb-head sticky right-0 bg-red-10 w-[165px] md:w-[236px] xl:relative xl:w-auto">상태</th>
            </tr>
          </thead>
          <tbody>
            {data.map((data) => (
              <tr className="text-[14px]  md:text-[16px] border-b border-gray-20 h-[52px]">
                <td className="sticky left-0 bg-white tb-data xl:relative">{data.col1}</td>
                <td className="tb-data ">{data.col2}</td>
                <td className="tb-data ">{data.col3}</td>
                <td className="sticky right-0 bg-white tb-data xl:relative xl:w-[100px]" aria-label="badge">
                  {type === 'applicantList' ? (
